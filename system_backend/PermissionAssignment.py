...
def SelectMenus():
    if request.method == 'GET':
        ...
        try:
            ...
            periss = db_session.query(Permission).filter(Permission.Name == current_user.Name,
        ...
        try:
            ...
            if MenuName == None:
                MenuNames = db_session.query(Permission.MenuName).filter(
                    Permission.WorkNumber == current_user.WorkNumber, Permission.MenuType == MenuType).all()
            else:
                ParentNode = db_session.query(ModulMenus.ID).filter(ModulMenus.ModulMenuName == MenuName).first()
                pmenus = db_session.query(ModulMenus.ModulMenuName).filter(ModulMenus.ParentNode == ParentNode,
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            for pid in permissionIDs:
                ...
                rpclas = db_session.query(RolePermission).filter(RolePermission.RoleID == roleclass.ID,
...
def selectpermissionbyuser():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            return json.dumps(
                {"code": "200", "message": "请求成功", "data": {"total": len(permission_list), "rows": permission_list}})
        except Exception as e:
            print(e)
            # logger.error(e)
