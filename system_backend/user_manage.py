...
from common.system_model import db_session, DepartmentManager, AreaMaintain, Role, RoleUser, User, ShiftsGroup, UserShiftsGroup
...
def get_user():
    ...
    for department in departments:
        ...
        for data in role_query:
            ...
            user_list = {'name': data.RoleName, 'value': data.RoleCode, 'role_description': data.Description,
                         'type': 'role', 'rid': data.ID, 'did': d1.ID, 'department_name': department.DepartName,
                         'children': []}
            ...
            for user_role in user_role_query:
                ...
                if user_query:
                    d2 = db_session.query(DepartmentManager).filter(
                        DepartmentManager.DepartName == user_query.OrganizationName).first()
                    if d2:
                        user_data = {'name': user_query.Name, 'value': user_query.WorkNumber, 'type': 'user',
                                     'rid': data.ID,
                                     'did': d2.ID}
                    else:
                        user_data = {'name': user_query.Name, 'value': user_query.WorkNumber, 'type': 'user',
        ...
        department_data = {'name': department.DepartName, 'value': department.DepartCode, 'type': 'department',
                           'did': department.ID, 'factory_name': factory.FactoryName, 'children': role_list}
    ...
    data = {'name': factory.FactoryName, 'value': factory.AreaCode, 'type': 'factory', 'fid': factory.ID,
            'children': queryset}
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            for pid in shiftsgroupIDs:
                ...
                rpclas = db_session.query(UserShiftsGroup).filter(UserShiftsGroup.UserID == userclass.ID,
                                                                  UserShiftsGroup.ShiftsGroupID == shiftsgroupcalss.ID).first()
                if not rpclas:
                    rp = UserShiftsGroup()
